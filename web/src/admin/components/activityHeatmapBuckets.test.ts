...
describe("buildHeatmapColumns", () => {
  ...
  it("covers the widest supported range at month scale", () => {
    // The 366-day cap (ActivityHeatmap's "Max" preset, and what the API
    // allows) starting mid-month spans 13 calendar months — every one of
    // them gets a column, including the partial months at each end.
    const start = Date.UTC(2025, 6, 21, 0, 0, 0) / 1000;
    const end = start + 366 * 24 * 60 * 60;
    const columns = buildHeatmapColumns(start, end, "month");
    expect(columns.length).toEqual(13);
    expect(columns[0].startKey).toEqual("2025-07-01");
    expect(columns[12].endKey).toEqual("2026-07-31");
  });
});

describe("nearestValidScale", () => {
  it("leaves a scale that already fits the range alone", () => {
    expect(nearestValidScale("day", MAX_DAY_SCALE_RANGE_DAYS)).toEqual("day");
    expect(nearestValidScale("month", 30)).toEqual("month");
    expect(nearestValidScale("week", MAX_RANGE_DAYS)).toEqual("week");
  });

  it("coarsens only as far as needed when the range outgrows the scale", () => {
    expect(nearestValidScale("day", MAX_DAY_SCALE_RANGE_DAYS + 1)).toEqual(
      "week",
    );
    expect(nearestValidScale("day", MAX_RANGE_DAYS)).toEqual("week");
  });

  it("never goes finer than the requested scale", () => {
    // A month-scale view of a week-sized range is valid, so it stands.
    expect(nearestValidScale("month", 7)).toEqual("month");
  });

  it("keeps the scale when nothing coarser fits either", () => {
    // Past the API's own cap no scale is valid — the caller surfaces the
    // range error instead of silently changing the view.
    expect(nearestValidScale("day", MAX_RANGE_DAYS + 1)).toEqual("day");
  });
});
