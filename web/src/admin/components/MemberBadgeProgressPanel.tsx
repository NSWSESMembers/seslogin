...
// Gold/silver/bronze/passport-stamps reuse the same border tokens as
// TIER_PILL_CLASS (app.css's --color-tier-*-border); starter intentionally
// stays a plain neutral (it was never colour-matched to its pill border in
// the original design). The inset shadows are one-off decoration, so they
// stay as arbitrary values (see Panel.tsx's PanelBox for the same exemption).
const TIER_CARD_BORDER_CLASS: Record<string, string> = {
  gold: "border-tier-gold-border shadow-[inset_0_0_0_1px_rgba(185,138,0,0.2)]",
  silver: "border-tier-silver-border",
  bronze: "border-tier-bronze-border",
  starter: "border-gray-400",
  "passport-stamps":
    "border-tier-passport-stamps-border shadow-[inset_0_0_0_1px_rgba(63,143,104,0.18)]",
};

const TIER_ICON_ANIMATION_CLASS: Record<string, string> = {
  gold: "animate-badge-tier-gold motion-reduce:animate-none",
  silver: "animate-badge-tier-silver motion-reduce:animate-none",
  bronze:
    "[transform-origin:center] animate-badge-tier-bronze motion-reduce:animate-none",
  starter: "animate-badge-tier-starter motion-reduce:animate-none",
};

const SEGMENT_BUTTON_CLASS =
  "m-0 min-w-23 cursor-pointer rounded-none border-0 bg-surface-raised px-3 py-1.5 text-ink hover:bg-surface-sunken aria-pressed:bg-navy aria-pressed:text-white aria-pressed:hover:bg-navy-active";
...
export default function MemberBadgeProgressPanel({
  ...
  return (
    <section className="mt-6 rounded-[10px] border border-[#d8d8d8] bg-linear-to-b from-[#fffaf4] to-white p-3.5">
      {heading ? (
        <h3 className="m-0 mb-2.5 text-badge-accent">{heading}</h3>
      ) : null}
      <div
      ...
      >
        <div className="flex min-w-21 flex-col justify-center rounded-xl border border-[rgba(154,108,50,0.2)] bg-white/74 px-3 py-2">
          <span className="text-[1.6rem] leading-none font-extrabold text-badge-accent">
            {earnedBadgeCount}
          </span>
        ...
        <div className="ml-auto flex flex-wrap items-center justify-end gap-3">
          <div className="inline-flex items-center gap-2">
            ...
            >
              ...
              <button
                type="button"
                className={`${SEGMENT_BUTTON_CLASS} border-l border-line-strong`}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex w-full flex-col items-center">
                      <div
                        className={`mb-px leading-[1.2] font-bold ${badge.earned ? "text-[#4e2f00]" : "text-ink-muted"}`}
                      >
                        {badge.name}
                      </div>
                      <div
                        className={`text-[0.88rem] capitalize ${badge.earned ? "text-[#7a4b00]" : "text-ink-muted"}`}
                      >
                        {badge.tier} tier
                      </div>
                      {badge.description ? (
                        <p
                          className={`mt-2 mb-2.5 ${badge.earned ? "text-[#493200]" : "text-ink-muted"}`}
                        >
                          {badge.description}
                        </p>
                      ) : null}
                      <div
                        className={`mt-0.5 text-[0.85rem] ${badge.earned ? "text-[#6b4200]" : "text-ink-muted"}`}
                      >
                        {badge.awardedAt ? (
                          <>
                            Earned{" "}
                          ...
                          >
                            <div className="h-1.25 flex-1 overflow-hidden rounded-full bg-black/10">
                              {/* This progress-bar fill is the same tan-to-gold
                                  gradient for every badge regardless of its own
                                  tier — from-tier-bronze-border is reused here
                                  only because it happens to be the same color,
                                  not because this is bronze-specific. */}
                              <div
                                className="h-full rounded-full bg-linear-to-r from-tier-bronze-border to-[#e0a81a]"
                                style={{
