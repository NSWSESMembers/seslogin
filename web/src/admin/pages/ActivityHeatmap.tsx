...
// Matches the server's cap on `periodSummaryByDayByMember` (a year, +1 day
// for a leap year) so an out-of-range request is rejected here rather than
// spending a round trip on it.
const MAX_RANGE_DAYS = 366;
// A day-scale column per day over a long range stops being a useful view —
// or a cheap one to render — long before MAX_RANGE_DAYS does, so day scale
// gets a tighter cap.
const MAX_DAY_SCALE_RANGE_DAYS = 90;
...
export default function ActivityHeatmap() {
  ...
  const maxRangeDays =
    scale === "day" ? MAX_DAY_SCALE_RANGE_DAYS : MAX_RANGE_DAYS;
