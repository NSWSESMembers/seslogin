...
// Confetti is pure decoration on a fixed warm backdrop, so these stay literal
// rather than becoming @theme tokens — nothing else can reuse them and they have
    ...
    >
      ...
      >
        {confettiPieces.map((piece) => (
          <span
            key={piece.id}
            className="absolute right-(--confetti-origin-right,auto) bottom-6.5 left-(--confetti-origin-left,28px) h-[calc(var(--confetti-size)*0.55)] w-(--confetti-size) origin-center animate-badge-confetti rounded-full bg-(--confetti-color) opacity-0 shadow-[0_0_10px_rgba(255,255,255,0.22)] motion-reduce:animate-none motion-reduce:opacity-0"
            style={piece.style}
          />
        ))}
      </span>
      <span className="fixed top-1/2 left-1/2 z-3 flex max-h-[calc(100vh-40px)] w-[min(880px,calc(100vw-32px))] -translate-1/2 flex-col items-stretch gap-3.5 overflow-auto rounded-3xl border-2 border-[rgba(255,228,170,0.45)] bg-linear-to-b from-[rgba(255,251,238,0.98)] to-[rgba(255,240,209,0.98)] px-6.5 pt-6 pb-5.5 shadow-[0_30px_80px_rgba(0,0,0,0.38),0_0_0_1px_rgba(255,255,255,0.5)_inset]">
        <span className="font-title text-sm font-bold tracking-[0.14em] text-celebrate-eyebrow uppercase">
          Badge awarded
        </span>
        <span className="font-title text-[clamp(2rem,4vw,3.6rem)] leading-[1.02] font-bold text-badge-accent [text-shadow:0_1px_0_rgba(255,255,255,0.7)]">
        ...
        <span className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-3">
          {props.badges.map((badge) => (
            <span
              key={badge.id}
              className="grid animate-badge-pop grid-cols-[54px_1fr] items-center gap-x-2.5 rounded-[18px] border-2 border-celebrate-border bg-linear-[120deg] from-[#fff3d8] via-[#ffe7c6] via-60% to-[#ffd59e] px-4 py-3.5 shadow-[0_10px_24px_rgba(212,124,47,0.22)] motion-reduce:animate-none"
            >
              ...
              <span className="min-w-0">
                <span className="block text-base font-bold text-celebrate-title">
                  New badge: {badge.name}
                </span>
                <span className="block text-base leading-[1.35] text-celebrate-body">
                  {badge.description
