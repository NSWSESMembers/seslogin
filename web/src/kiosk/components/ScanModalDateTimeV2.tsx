...
// Index of the digit the orange ring sits on, or null for "no ring": either all
// four are entered, or the caret has run off the right-hand end.
type Caret = 0 | 1 | 2 | 3 | null;

const EMPTY_DIGITS: Digits = [null, null, null, null];
...
// Where the ring starts: on the first blank digit, or nowhere if the field
// opened already full (which it normally does — it prefills with the current
// time).
function initialCaret(digits: Digits): Caret {
  const first = digits.findIndex((d) => d === null);
  return first === -1 ? null : (first as Caret);
}
...
// The orange caret ring: on exactly one digit, and on none once all four are in.
// The border *colour* lives entirely in these two, never in digitBoxBase — two
// border-colour utilities in one class string resolve by stylesheet order rather
// than by the order they are written, so the ring would lose to the transparent
// default.
...
export function Inner(props: {
  ...
  function button(key: string) {
    ...
    // With no ring showing, the field is full (it opens prefilled with the
    // current time) — a digit starts a fresh entry rather than being a no-op.
    if (caret === null) {
      if (!isValidDigit(EMPTY_DIGITS, 0, digit)) return;
      setDigits([key, null, null, null]);
      setCaret(1);
      return;
    }
    ...
    next[caret] = key;
    setDigits(next);
    // Move right; past the last digit there is nowhere to go, so the ring clears.
    setCaret(caret === 3 ? null : ((caret + 1) as Caret));
  }

  // Backspace: clear the digit under the ring if there is one, otherwise the one
  // to its left. With no ring (all four entered) it clears the last digit.
  function del() {
    const target =
      caret === null ? 3 : digits[caret] !== null ? caret : caret - 1;
    if (target < 0) return;
    const next = [...digits] as Digits;
    next[target] = null;
    setDigits(next);
  ...
  function moveCaret(delta: -1 | 1) {
    if (caret === null) {
      // The ring is off the right-hand end; left re-enters at the last digit.
      if (delta === -1) setCaret(3);
      return;
    }
