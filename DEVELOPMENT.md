...
**It starts completely empty.** Nothing is copied from anywhere; there is no member,
location, or user data until you create some.

### Logging in

There's no seeded user yet, so the way in is the dev auth bypass — see
[§7](#7-bypassing-auth-for-local-ui-work), using an id you created yourself. Once a user
exists, the email-code login flow works end to end: request a code and read it out of the
API's log, where `mockmail` prints the whole message.
