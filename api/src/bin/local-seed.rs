...
//!
//!   extract  Read the synthetic test records out of a real database (needs AWS
//!            access) and write them to a checked-in fixture. Run rarely.
...
#[derive(Subcommand)]
enum Command {
    /// Read test records from a real database and write the fixture. Needs AWS access.
    Extract {
        /// Database to read from. Read-only; nothing is ever written to it.
        #[arg(long, default_value = "seslogin_prod")]
        source_prefix: String,

        /// Location whose record and members are extracted.
        #[arg(long, default_value = "wBsJHYxy9snR")]
        location: String,
...
async fn query_index(
    ...
    loop {
        let resp = client
            ...
            .send()
            .await
            .with_context(|| format!("querying {table}.{index}"))?;
        for item in resp.items() {
            out.push(item_to_json(item)?);
        }
        start_key = resp.last_evaluated_key().cloned();
...
async fn extract(source_prefix: &str, location_id: &str, allow_personal: bool) -> Result<()> {
    ...
    let location = client
        .get_item()
        .table_name(t("location"))
        .key("id", AttributeValue::S(location_id.to_string()))
        .send()
        .await
        .with_context(|| format!("reading location {location_id}"))?
        .item
        .ok_or_else(|| anyhow!("location {location_id} not found in {source_prefix}"))?;

    let (persons, dropped_persons) = drop_deleted(
        query_index(
            &client,
            &t("person"),
            "location_id-index",
    ...
    let dropped = dropped_persons + dropped_categories + dropped_groups;

    let mut tables = Map::new();
    tables.insert("location".into(), json!([item_to_json(&location)?]));
    tables.insert("person".into(), json!(persons));
...
async fn main() -> Result<()> {
    ...
    match Cli::parse().command {
        Command::Extract {
            ...
            location,
            allow_personal_data,
        } => extract(&source_prefix, &location, allow_personal_data).await,
